...
dsh plugin --profile web add ./dshplugin/dsh-mobile-ui/dist/dsh-mobile-ui-0.3.5.tgz  # 仓库里的 tarball
...
```sh
npm pack --pack-destination dist        # 生成 dist/dsh-mobile-ui-0.3.5.tgz
```

把 tarball 发给对方，对方执行：

```sh
dsh plugin --profile web add ./dsh-mobile-ui-0.3.5.tgz
systemctl --user restart dsh-web
```
