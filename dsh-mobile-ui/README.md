...
## 0.3.0 全量回归（2026-09-25）

对运行中的本机 GUI 做了一轮覆盖全部功能的实测（鸿蒙 390×844，真实长会话；另开桌面 1440×900 对照）：

| 检查项 | 结果 |
| --- | --- |
| UA 判定 | 鸿蒙 UA 生效；桌面 UA 完全不动（无 `data-dshm`、无把手/浮钮、官方 1440 布局）✓ |
| 侧栏把手 | `position: fixed` 于 (0, 6)，28×28；点击开抽屉，点遮罩关闭（把手在抽屉打开时隐藏，属设计）✓ |
| 三轨模板 | `0px minmax(0px, 1fr) minmax(0px, 0px)`，会话吃满宽度 ✓ |
| 输入区收展 | 卡片收起高 0、展开高 52；`--dshm-card-bottom` 26px / 297px 自适应 ✓ |
| 收缩钮与发送钮共中线 | 实测两态中线差 **0px** ✓ |
| 回到最新浮钮 | 高度恒为 712（= 工具列「模型」钮水平中线），横向随态在 301↔356 滑动；刷新后收起态上滑即出现 ✓ |
| 工具列四点 | 36×32、间隙 6px、列 x=338（展开）/354（收起，带退场位移）✓ |
| 右侧让位 | `--dshm-rail-inset: 42px` → 卡片右缘 330、工具列左缘 338，净空 **8px**，不重叠 ✓ |
| 常驻统计行 | 卡片下方 4px 常驻，两态都在 ✓ |
| 顶部选项卡 | `[data-phase] header [role=tablist]` 隐藏（display: none），切换项进入 ⋯ 菜单 ✓ |
| 头部 ⋯ 菜单 | 实收条目：Trajectory / Download session / Feedback ✓ |
| 右侧滚动条 | `scrollbar-width: none`，`offsetWidth - clientWidth = 0` ✓ |
